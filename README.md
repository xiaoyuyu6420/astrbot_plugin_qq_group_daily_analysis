...
本插件会读取群聊消息、提取有价值信息、并可能转发给管理员或上传到群相册/群文件。**部署者有责任确保符合当地法律法规和群规**，建议：

- **告知群成员**：在群里发布公告，告知成员"本群已部署日报机器人，会自动汇总群消息并推送给管理员"。一个简短示例：
  > 📢 本群已启用日报机器人，每天会自动分析群聊内容并生成摘要给管理员。如不愿被记录，请联系管理员调整。
- **控制数据流向**：默认情况下，分析摘要只发送给配置的管理员（`extra_admin_qq`）。开启 `enable_admin_notify` 后**不会**主动发到群里。如开启了上传群相册/群文件，请评估是否合适。
- **保留期**：插件内置 `basic.retention_days` 配置（默认 30 天），到期会自动清理：
  - 历史日报摘要（KV 存储）
  - debug 模式 dump 的群聊原文 JSON（默认 7 天，仅 debug 模式开启时才会生成）
  - 增量分析批次（按 `incremental_interval_minutes` 自动滚动）
  
  设为 `0` 可关闭自动清理（不推荐，长期运行会让 KV 存储无限增长）。
- **debug 模式谨慎开启**：`basic.debug_mode = true` 会把每次 LLM 分析的完整输入（含昵称、用户 ID、消息原文）写到磁盘用于排查。**生产环境请保持关闭**。
- **手动清理**：执行 `/分析设置 reload` 会触发一次过期数据清理。
- **个人信息最小化**：日报摘要默认只包含统计与话题文本，不含头像/原始消息流。头像仅在图片/PDF 报告中渲染，可在配置中关闭。
